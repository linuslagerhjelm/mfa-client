...
export const startRegistration = (email: string, password: string) => 
  new Promise<string>((resolve, reject) => {
    axios
      ...
      .catch(err => handleReject(err, reject));
  });
...
const handleReject = (err: any, reject: any) => 
  reject({
    message: err?.response?.data?.error || err?.response?.data?.message || "An error occurred but the server did not provide an error message",
